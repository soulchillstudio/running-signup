...
function wrongClassNotice_() {
  ...
  <p style="margin:14px 0;padding:12px 14px;background:#fdf1ea;border-left:3px solid #b8542a;border-radius:6px;font-size:14px">
    <b>班級不對嗎？</b>直接回覆這封信告訴我們正確的班別（台中週二 / 台北週三），我們會幫您改，不用重填。
  </p>`;
}
...
const CLASS_MAP = {
  'taichung-tue': {
    ...
    // 2026-09-04 更正日期：台中團練隨週四班停開改到週二 9/08、9/15。
    warmups: '9/08、9/15 · 週二'
  },
  'taipei-wed': {
  ...
  // 🔴 2026-09-04 台中週四班停開（Jesse 決定收掉，週四晚上挪去做 Premium 線上招生）。
  //    報名頁已經沒有這個選項，doPost 收不到 class=taichung-thu。
  //    這一筆刻意保留：既有三位夥伴的列若還留在「台中週四班」分頁，
  //    onStatusEdit 才認得出那個分頁、報名成功信才寄得出去。
  //    等三列都搬到「台中週二班」分頁之後，這一整段可以刪。
  'taichung-thu': {
    tab: '台中週四班', label: '台中 · 週四班（已停開）', location: '中興大學田徑場',
...
// 🔴 2026-09-03 起這張表是死的:對外開放的體驗場已結束、T 方案已從報名頁移除,
//    唯一讀它的 trialEmail() 不會再被呼叫。日期停在舊值,不要拿它當真相。
//    現行團練:台中 9/08、9/15(週二)/ 台北 9/09、9/16(週三,台大田徑場)
const TRIAL_SESSION = {
